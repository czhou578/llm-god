(() => {
  ...
  let debounceTimer: NodeJS.Timeout; // Timer for debouncing the click action
  ...
  const onGenerationFinish = () => {
    ...
    if (lastMessageGroup) {
      ...
      const targetButton = lastMessageGroup.querySelector(targetButtonToClickSelector.replace('div.text-base:last-of-type ', '')) as HTMLButtonElement; // Adjust selector for context
      if (targetButton) {
